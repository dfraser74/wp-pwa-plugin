...
(function(document, window, navigator) {
  ...
  var device = 'desktop';
  if (navigator && isMobile(navigator.userAgent)) device = 'mobile';
  else if (navigator && isTablet(navigator.userAgent)) device = 'tablet';
  ...
  if (readCookie('wppwaClassicVersion')) {
    var options = {
      ...
      id: 'wppwaClassic',
      src:
        window['wp-pwa'].ssr + 'dynamic/wp-org-connection-app-extension-worona/go-back-to-wppwa.js',
    };
    loadScript(options);
  } else if (!readCookie('wppwaInjectorFailed') && device !== 'desktop') {
    window.stop();
    ...
    */
    ...
    var query =
      '?siteId=' +
      window['wp-pwa'].siteId +
      '&static=' +
      window['wp-pwa'].static +
      '&env=' +
      window['wp-pwa'].env +
      '&device=' +
      device;
